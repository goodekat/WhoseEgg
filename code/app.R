...
ui <- dashboardPage(
  ...
  # App title ----
  dashboardHeader(title = "Decide on a name for the app", titleWidth = 375),
  
  # Sidebar panel for inputs ----
  dashboardSidebar(
    
    # Format options for sidebar
    width = 375,
    
    # Sidebar tabs
    sidebarMenu(
      menuItem("Questions", tabName = "questions"),
      menuItem("Overview", tabName = "overview"),
      menuItem("Glossary", tabName = "glossary"),
      menuItem("Random forest predictions", tabName = "rf"),
      h2("Input values:"),
      selectInput(
        inputId = "input_opt",
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(
        tabName = "overview",
        p("Add information about the app: information on the random forest model, how to use the app, previous papers, etc.")
        ),
      
      # Current questions tab
      tabItem(
        tabName = "glossary",
        p("Place to put glossary for egg characteristics")
      ),
...
server <- function(input, output) {
    ...
    # Create plots with the random forest probabilites for all taxonomic levels
