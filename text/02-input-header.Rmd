...
The egg characteristic data must be formatted appropriately to work with WhoseEgg and correctly obtain predictions. Follow the guidelines in the **Spreadsheet Specifications** tabs below. Once the egg characteristic spreadsheet is uploaded, several additional variables will be computed based on the input values to be used by the random forests: `Julian_Day`, `Membrane_SD`, `Membrane_CV`, `Yolk_SD`, `Yolk_CV`, and `Yolk_to_Membrane_Ratio`. The uploaded variables of `Year` and `Day` are only used by WhoseEgg to compute `Julian_Day` and are excluded from the processed data.
